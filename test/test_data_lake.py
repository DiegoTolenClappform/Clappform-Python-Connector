...
class TestDataFrame(unittest.TestCase):
    ...
    def test_getAddPermissions(self):
        ...
        resp = App("default").Collection(self.collection_id).DataFrame().Synchronize(dataframe=TestDataFrame.global_dataframe_data, n_jobs=0)
        assert resp is True
        print("=====[ Done Syncing collection and dataframe ]=====")

    def test_deleteApp(self):
        print("=====[ Appending collection and dataframe ]=====")
        # We first need to create a app before we can delete one
        App("default").Collection(self.collection_id).DataFrame().Append(dataframe=TestDataFrame.global_dataframe_data, n_jobs=1, show=False)
    ...
    def test_deleteCollection(self):
        ...
        # We first need to create a app and collection before we can delete one
        App("default").Collection(self.collection_id).DataFrame().Append(dataframe=TestDataFrame.global_dataframe_data, n_jobs=1, show=False)

        self.dataframe_query = App("default").Collection(self.collection_id).DataFrame().Query(filters={}, projection={}, sorting={}, original=True)
